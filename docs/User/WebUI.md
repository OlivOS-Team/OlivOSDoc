...
WebUI 端口被占用时会自动向后顺延，例如 `20480` → `20481` → `20482`，直到找到可用端口（最多到 `65535`）。启动日志和 Windows 托盘“打开 WebUI”使用实际端口，不会改写用户配置。多个 OlivOS 实例应使用独立工作目录；此行为仅适用于 WebUI，不改变机器人协议端的端口。使用 SSH 转发时，远端端口也应填写日志中的实际端口。
...
“插件页面”分组可折叠，并记住折叠状态。宽屏侧栏中，入口较多时插件列表内部竖向滚动，不会带动 Logo、主导航和外观设置；横屏窗口高度不足以放下固定导航时，侧栏整体也可独立滚动，保证所有入口仍可访问。右侧主内容超出高度时显示自己的滚动条，与侧栏互不影响。窄屏或竖屏时导航移至顶部，插件入口单行排列、横向滚动，不产生插件列表的竖向滚动条，页面内容仍可正常上下滚动。
